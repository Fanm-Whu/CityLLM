...
density_path = r'C:\Users\Zhongym\Desktop\多要素强耦合模型\实验结果\GPY预测人口密度_2030缓慢.tif'
...
# 基础参数设置 # 自然断点-八类别
max_increase = 15461009
breaks =  [1,5,20,50,100,200,300,400,550]
density_break=[0.1364, 0.3093, 0.3592, 0.4572, 0.5792, 0.6347, 0.7572, 1]
...
def weighted_allocation(rowss, colss, density_data, values_range_data):
    # Calculate weights based on the densities at specified indices
    weights = density_data[rowss, colss]
    exponential_weights = np.power(weights, 3)

    # Sum up the weights
    weight_sum = np.sum(exponential_weights)

    # Handle the case where the sum of weights is zero to avoid NaN probabilities
    if weight_sum == 0:
        # Assign equal probabilities if all weights are zero
        normalized_weights = np.ones(len(exponential_weights)) / len(exponential_weights)
    else:
        normalized_weights = exponential_weights / weight_sum

    # Ensure values_range_data is sorted in descending order
    values_range_data = np.sort(values_range_data)[::-1]

    # Allocate values based on the weighted probabilities
...
def allocate_population(density, density_break, class_means, max_increase,Total_tolerance_error=max_increase*0.05):
    ...
    df_error_values.to_excel(r'C:\Users\Zhongym\Desktop\多要素强耦合模型\输出文档和图\error_values_2030_pop缓慢.xlsx', index=False)
    ...
    plt.savefig(r'C:\Users\Zhongym\Desktop\多要素强耦合模型\输出文档和图\error_values_2030_pop缓慢.png')
    plt.show()
...
output_file = r'C:\Users\Zhongym\Desktop\多要素强耦合模型\实验结果\final_allocated_population_2030缓慢.tif'
