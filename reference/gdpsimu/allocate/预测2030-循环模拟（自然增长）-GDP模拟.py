...
dataset3 = gdal.Open(r'C:\Users\Zhongym\Desktop\多要素强耦合模型\实验结果\2030年土地功能模拟结果_自然.tif')
land = dataset3.GetRasterBand(1).ReadAsArray().astype(float)
data_path = r'C:\Users\Zhongym\Desktop\多要素强耦合模型\操作数据\GDP2019_wuha.tif'
density_path = r'C:\Users\Zhongym\Desktop\多要素强耦合模型\实验结果\GPY预测GDP密度_2030自然.tif'
...
# 基础参数设置 # 自然断点-八类别
# max_increase =20459029757300
# breaks =  [0,4036324.95893039,12757171.1875471,25478927.1555916,46673667.9008242,76506191.316332,98863956.2276825,154159965.351367,271333764.011531]

max_increase = 253686009
breaks = [0,100,300,500,1000,1500,2000,3000,3521]
# density_break=[0.127625033,0.303389367,0.326514939,0.350682368,0.405891655,0.560545895,0.663048183,1]
density_break=[0.107625033,0.303389367,0.326514939,0.350682368,0.405891655,0.500545895,0.633048183,1]
indice_map=np.array([i for i in range(0,2331*2724)]).reshape(2331,2724)
print('检查输入各类别数组长度',len(density_break),len(breaks))
allocation_attempts_times= 70 # 总迭代时间
did_coff=6
print(np.sum(data))
...
def allocate_population(density, density_break, max_increase,Total_tolerance_error=max_increase*0.05):
    ...
    while allocation_attempts < allocation_attempts_times or (len(error_values) > 5 and np.std(error_values[-5:]) < 2000):
        ...
        with tqdm(total=max_increase) as pbar:  # 初始化进度条
            ...
            for i, mean in enumerate(density_break):
        ...
        if np.abs(total_population_allocated - max_increase)>Total_tolerance_error :
            ...
            error_value=np.abs(total_population_allocated - max_increase) - Total_tolerance_error
            print('当前GDP为',total_population_allocated ,'GDP误差为',error_value/max_increase)
            print('目前密度划分区间为', density_break)
        else:
            error_value = np.abs(total_population_allocated - max_increase) - Total_tolerance_error
            print('最终GDP误差为', error_value/max_increase)
    ...
    df_error_values.to_excel(r'C:\Users\Zhongym\Desktop\多要素强耦合模型\输出文档和图\error_values_GDP_2030_自然.xlsx', index=False)
    ...
    plt.savefig(r'C:\Users\Zhongym\Desktop\多要素强耦合模型\输出文档和图\error_values_GDP_2030自然.png')
    plt.show()
...
adjusted_allocated_population = allocate_population(density, density_break,  max_increase)


# 保存结果到栅格文件
driver = gdal.GetDriverByName('GTiff')
output_file = r'C:\Users\Zhongym\Desktop\多要素强耦合模型\实验结果\final_allocated_gdp_2030自然2.tif'
